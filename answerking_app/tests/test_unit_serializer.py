...
class SerializerTests(TestCase):
    ...
    def test_cat_det_serializer_desc_field_content(self):
        data: ReturnDict = self.TB.test_cat_det_serializer.data
        self.assertEqual(data['description'], self.TB.test_cat_1.description)
    ...
    def test_cat_det_create_fn_no_products_pass(self, products_check_mock):
        ...
        new_cat = cds.create(self.TB.cat_det_serializer_data)
        created_cat = Category.objects.get(pk=new_cat.id)
        products_check_mock.assert_called_once()
        self.assertEqual(created_cat.name, self.TB.cat_det_serializer_data['name'])
        self.assertEqual(created_cat.description, self.TB.cat_det_serializer_data['description'])
    ...
    def test_cat_det_create_fn_w_products_pass(self, products_check_mock):
        ...
        new_cat = cds.create(self.TB.cat_det_serializer_data)
        created_cat = Category.objects.get(pk=new_cat.id)
        expected_prod_ids = [self.TB.test_product_1,
                             self.TB.test_product_2
                             ]
    ...
    def test_cat_det_update_fn_retired_category_fail(self, products_check_mock):
        with self.assertRaises(ProblemDetails) as cm:
            retired_cat = self.TB.test_cat_retired
            cds = CategoryDetailSerializer()
            cds.update(retired_cat, self.TB.cat_det_serializer_data)
