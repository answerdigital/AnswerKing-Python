...
class LineItemSerializerUnitTests(UnitTestBase):
    UTB = UnitTestBase()
    prod_data: dict = UTB.get_fixture(
        "products",
    ...
    def setUp(self):
        ...
        line_item: LineItem = LineItem.objects.create(order=order, product=prod, quantity=self.quant)
    ...
    def test_line_item_serializer_contains_correct_fields(self):
        ...
        test_line_item: LineItem = test_order.lineitem_set.get(order=test_order)
    ...
    def test_line_item_serializer_product_content(self):
        ...
        self.assertEqual(list(expected.category_set.all()), actual['categories'])
    ...
    def test_line_item_serializer_quantity_less_than_zero_fail(self):
        serializer_data = {"product": self.serialized_product_detail,
                           "quantity": -1
                           }
    ...
    def test_line_item_serializer_quantity_greater_than_max_val_fail(self):
        ...
        self.assertEqual(len(serializer.errors), 1)
        self.assertIn("quantity", serializer.errors.keys())
        self.assertEqual(f"Ensure this value is less than or equal to {MAXNUMBERSIZE}."
                         , serializer.errors['quantity'][0])
