...
class ModelTests(UnitTestBase):
    ...
    def test_calculate_total_with_line_items(self):
        ...
        test_order_line_1 = LineItem.objects.create(order=test_order, product=prod_1, quantity=quant_1)
        test_order_line_2 = LineItem.objects.create(order=test_order, product=prod_2, quantity=quant_2)
        ...
        expected_tot: Decimal = Decimal(
            (prod_1.price * quant_1)
            + (prod_2.price * quant_2)
    ...
    def test_calculate_total_without_sub_total_calculated(self):
        ...
        quant = 1
        test_order: Order = Order.objects.create()
        test_order_line_1 = LineItem.objects.create(order=test_order, product=prod, quantity=quant)
        test_order.calculate_total()
